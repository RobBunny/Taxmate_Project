...
GENERAL_FINANCE_KB = {
    ...
    "what are sip risks": "SIP risks include market risk (fund value can fall), rupee cost averaging doesn't guarantee profits, fund manager risk, and the discipline required to continue investments during market downturns. However, SIPs help mitigate timing risk through regular investing.",
    "sip risks": "Key SIP risks include: 1) Market volatility affecting returns, 2) No guaranteed returns, 3) Fund manager performance risk, 4) Need for long-term discipline, 5) Inflation risk over time. Despite risks, SIPs are generally considered safer than lump-sum investing due to rupee cost averaging.",
    "risks of sip": "SIP carries several risks: market risk (your investments can lose value), no guarantee of returns, fund manager performance risk, and the need for consistent investment discipline. However, SIPs help reduce timing risk and volatility impact compared to lump-sum investments.",
    ...
    "what is ira": "IRA (Individual Retirement Account) is a tax-advantaged retirement savings account. Traditional IRA offers tax deductions now but you pay taxes on withdrawals. Roth IRA uses after-tax money but withdrawals are tax-free in retirement.",
    "what is amortization": "Amortization is the process of paying off debt through regular payments over time. Each payment covers both interest and principal, with the principal portion increasing over time while interest decreases.",
    "financial help": "I can help with budgeting, investing, retirement planning, debt management, insurance needs, tax planning, and general financial advice. What specific area would you like assistance with?",
    "what is budget": "A budget is a plan for your income and expenses over a specific period. It helps you track where your money goes, control spending, and achieve financial goals by allocating money to different categories like housing, food, entertainment, and savings.",
    "what is budgeting": "Budgeting is the process of creating and following a spending plan. It involves tracking your income and expenses, setting financial goals, and making informed decisions about how to allocate your money effectively.",
    "budget help": "I can help you create a budget using the 50/30/20 rule: 50% for needs (housing, utilities, groceries), 30% for wants (entertainment, dining out), and 20% for savings and debt repayment. Would you like specific budgeting tips or help analyzing your current spending?",
    "how to budget": "To create a budget: 1) Track your income and expenses for a month, 2) Categorize expenses (needs vs wants), 3) Use the 50/30/20 rule as a starting point, 4) Set realistic spending limits, 5) Review and adjust monthly. Start with tracking to understand your spending patterns.",
    "budgeting tips": "Key budgeting tips: 1) Track every expense for awareness, 2) Pay yourself first (save before spending), 3) Use the envelope method for cash spending, 4) Review and adjust monthly, 5) Plan for irregular expenses, 6) Automate savings and bill payments, 7) Be realistic with your limits.",
    "budget": "A budget helps you plan and control your spending. Follow the 50/30/20 rule: allocate 50% of income to needs (rent, utilities, groceries), 30% to wants (entertainment, dining), and 20% to savings and debt payment. Track expenses to see where your money goes."
}
...
class AdvancedFinancialChatbot:
    ...
    def _detect_financial_focus(self, message: str) -> Optional[str]:
        ...
        if matches:
            ...
            span = doc[start:end]
            # Map financial terms to categories
            finance_categories = {
                "investments": ["mutual fund", "sip", "stocks", "bonds", "investment"],
                "retirement": ["401k", "ira", "retirement", "pension"],
                "debt": ["loan", "credit", "mortgage", "amortization"],
                "insurance": ["life insurance", "health insurance", "insurance"],
                "planning": ["budget", "savings", "emergency fund", "financial planning"]
            }
            
            for category, terms in finance_categories.items():
                if any(term in span.text.lower() for term in terms):
                    return category
        return None
    ...
    def _answer_general_finance_question(self, message: str) -> Optional[str]:
        ...
        if any(token.text.lower() in ["what", "define", "definition"] for token in doc):
            for ent in doc.ents:
                # Check if entity is in our financial terms
                if ent.text.lower() in [term.lower() for term in self.financial_terms.keys()]:
    ...
    def get_definition(term: str) -> Optional[str]:
        ...
        financial_terms_dict = {
            ...
            "NAV": "Net Asset Value - The per-share value of a mutual fund, calculated by dividing total assets minus liabilities by number of shares.",
            "Expense Ratio": "The annual fee charged by mutual funds, expressed as a percentage of your investment.",
            "Budget": "A plan for your income and expenses over a specific period, helping you manage money and achieve financial goals.",
            "Budgeting": "The process of creating a plan to spend your money wisely, tracking income and expenses to ensure you live within your means."
        }
        
        for key, value in financial_terms_dict.items():
